...
def evaluate_forward_fn(args, forward_str):
    ...
    df_val = pandas.read_csv(args.val_file)
    df_test = pandas.read_csv(args.test_file)

    if SEARCHING_MODE:

        random.seed(args.shuffle_seed)
        examples = [row.to_dict() for _, row in df_val.iterrows()]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--expr_name', type=str, default="medqa_gpt3.5_results")
