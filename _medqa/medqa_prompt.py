...
base = """# Overview
You are an expert machine learning researcher testing various agentic systems. Your objective is to design building blocks such as prompts and control flows within these systems to solve complex tasks. Your aim is to design an optimal agent performing well on the MedQA dataset benchmark, a challenging evaluation that assesses a model's ability to answer questions from the United States Medical Licensing Examination (USMLE). 

The dataset contains multiple-choice questions (MCQs) which cover a wide range of medical topics, including basic sciences, clinical knowledge, and patient management. The questions typically involve complex reasoning and require a deep understanding of medical science. They are often scenario-based, presenting a clinical vignette followed by a question and multiple possible answers. Each question is accompanied by four answer choices, only one of which is correct.


## An example question from MedQA:
